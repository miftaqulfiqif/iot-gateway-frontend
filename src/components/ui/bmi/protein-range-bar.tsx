...
type ProteinRangeBarProps = {
  value: number;
  onChangeValue: (info: { label: string; color: string }) => void;
};

const ProteinRangeBar: React.FC<ProteinRangeBarProps> = ({
  value,
  onChangeValue,
}) => {
  const min = 10;
  ...
  const tickValues = [proteinRanges[0].from, ...proteinRanges.map((r) => r.to)];
  ...
  const category =
    ...
    ) || proteinRanges[proteinRanges.length - 1];
  ...
  return (
    <div className="relative w-full px-4 mt-4">
      ...
      <div className="relative w-full h-4 mt-1">
        {tickValues.map((tick) => {
          const min = proteinRanges[0].from;
          const max = proteinRanges[proteinRanges.length - 1].to;
...
export default ProteinRangeBar;
